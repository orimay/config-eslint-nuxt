...
export function fromNuxtConfigComposer(
  withNuxt: (...configs: ResolvableFlatConfig[]) => FlatConfigComposer,
) {
  const configNew: ResolvableFlatConfig = {};

  for (const cfg of config) {
    configNew.files = [...(configNew.files ?? []), ...(cfg.files ?? [])];
    configNew.ignores = [...(configNew.ignores ?? []), ...(cfg.ignores ?? [])];
    configNew.plugins = { ...configNew.plugins, ...cfg.plugins } as Record<
      string,
      ESLint.Plugin
    >;
    ...
    configNew.linterOptions = {
      ...configNew.linterOptions,
      ...cfg.linterOptions,
    } as Linter.LinterOptions;
  }

  configNew.rules = {
    ...(configNew.plugins?.['@stylistic'].rules as object),
    ...(configNew.plugins?.['@typescript-eslint'].rules as object),
    ...configNew.rules,
  };
